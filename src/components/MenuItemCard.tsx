...
const MenuItemCard: React.FC<MenuItemCardProps> = ({ item }) => {
  ...
  const startingPrice = hasVariants 
    ? Math.min(item.basePrice, ...item.variants.map((v: any) => item.basePrice + v.priceDelta))
    : item.basePrice;
      ...
      <Link to={`/item/${item.slug}`}>
        ...
        <div className="p-6">
          ...
          <div className="flex items-center justify-between">
            ...
            <button className="bg-orange-500 text-white p-2 rounded-full hover:bg-orange-600 transition-colors group-hover:scale-110 transform duration-200">
